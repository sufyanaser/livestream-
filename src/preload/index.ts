...
import { OBS_CHANNELS, type ObsBridge, type ObsConnectionConfig, type ObsLayoutSource, type ObsState } from '../shared/obs'
...
contextBridge.exposeInMainWorld('nasBroadcast', {
  ...
  runtime: 'electron',
  system: { copyText: (value: string) => ipcRenderer.send('system:copy-text', value) },
  project: {
  ...
  obs: {
    ...
    selectScene: (sceneName: string) => ipcRenderer.invoke(OBS_CHANNELS.selectScene, sceneName) as Promise<ObsState>,
    take: () => ipcRenderer.invoke(OBS_CHANNELS.take) as Promise<ObsState>,
    setTransition: (name: string, duration: number) => ipcRenderer.invoke(OBS_CHANNELS.setTransition, name, duration) as Promise<ObsState>,
    ensureGraphics: (endpoint: string) => ipcRenderer.invoke(OBS_CHANNELS.ensureGraphics, endpoint) as Promise<ObsState>,
    applyLayout: (layoutName: string, sources: ObsLayoutSource[]) => ipcRenderer.invoke(OBS_CHANNELS.applyLayout, layoutName, sources) as Promise<ObsState>,
