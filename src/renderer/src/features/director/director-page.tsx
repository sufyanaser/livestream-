...
export function DirectorPage({ project, obsState, connected }: { project: ProjectDocument; obsState: ObsState; connected: boolean }): React.JSX.Element {
  ...
  return (
    <main className="grid min-h-0 flex-1 grid-cols-[230px_minmax(540px,1fr)_245px] gap-3 p-3">
      <aside className="grid min-h-0 grid-rows-[minmax(260px,1fr)_auto] gap-3"><SourcePanel connected inputs={connected ? obsState.inputs : offlineInputs} scenes={connected ? obsState.scenes : offlineScenes} /><LayoutPanel /></aside>
      <section className="flex min-w-0 flex-col gap-3">
        ...
        <div className="grid grid-cols-[1fr_300px] gap-3"><LowerThirdPanel /><TransitionPanel /></div>
